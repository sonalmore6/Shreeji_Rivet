...
export default function SelfClinchingFastenerDetail() {
  const { id } = useParams();
  const product = selfClinchingProducts.find((p) => p.id === id);
  ...
  if (!product) {
    return (
      <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 20, padding: 40 }}>
        ...
        <Link to="/self-clinching-fasteners" className="btn-sm-blue">← Back to Self-Clinching Fasteners</Link>
      </div>
    );
  }
  ...
  return (
    <div>
      <SEO
        title={`${product.name} — Self Clinching Fastener`}
        description={product.desc}
        keywords={`${product.name}, Clinch Nut, Rosan Nut, Self Clinch Nut, Standoff, Shreeji Rivet India`}
        path={`/self-clinching-fasteners/${product.id}`}
        ...
        schema={{
          ...
          category: 'Self Clinching Fastener',
        }}
      />

      {/* ══ HERO ══ */}
      <section style={{ background: DARK_BG, padding: '80px 0 70px', position: 'relative', overflow: 'hidden' }}>
        <div className="blob blob-abs" style={{ width: 500, height: 500, top: '-30%', right: '-8%', background: 'radial-gradient(circle,rgba(124,58,237,.15) 0%,transparent 70%)' }} />
        ...
        <div className="container-xl" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 28, fontSize: '.8rem', flexWrap: 'wrap' }}>
            ...
            <Link to="/self-clinching-fasteners" style={{ color: '#38BDF8', textDecoration: 'none' }}>Self-Clinching Fasteners</Link>
            <span style={{ color: 'rgba(255,255,255,.3)' }}>/</span>
          ...
          <div className="row align-items-center g-5">
            ...
            <div className="col-12 col-md-5 s-hide-l">
              <div style={{
                ...
                <img src={product.img} alt={product.name} style={{ maxHeight: 260, maxWidth: '100%', objectFit: 'contain', filter: 'drop-shadow(0 8px 32px rgba(124,58,237,.2))' }} />
              </div>
            </div>
            ...
            <div className="col-12 col-md-7 s-hide-r">
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 36 }}>
                {product.chips.map((chip) => (
                  <span key={chip} style={{
                    ...
                    background: LIGHT, border: `1px solid rgba(124,58,237,.3)`,
                    color: SOFT,
                  }}>{chip}</span>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
                ...
                <Link to="/self-clinching-fasteners" style={{
                  ...
                  onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(196,181,253,.5)'; e.currentTarget.style.color = SOFT; }}
      ...
      {/* ══ SPECIFICATIONS ══ */}
      <section style={{ padding: '80px 0', background: '#F5F3FF' }}>
        <div className="container-lg">
          <div className="text-center s-hide mb-5">
            ...
            {product.specs.map((s, i) => (
              <div key={s.title} className={`col-12 col-md-4 s-hide d${i + 1}`}>
                <div style={{
                  ...
                  onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-6px)'; e.currentTarget.style.boxShadow = '0 20px 50px rgba(124,58,237,.12)'; e.currentTarget.style.borderColor = 'rgba(124,58,237,.35)'; }}
            ...
            {product.features.map((f, i) => (
              <div key={f.title} className={`col-12 col-md-4 s-hide d${i + 1}`}>
                <div style={{
                  ...
                  onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-5px)'; e.currentTarget.style.borderColor = 'rgba(124,58,237,.35)'; }}
                  onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'rgba(124,58,237,.12)'; }}
                >
      ...
      <section style={{ background: DARK_BG, padding: '70px 0', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div className="blob2 blob-abs" style={{ width: 400, height: 400, top: '-40%', left: '50%', transform: 'translateX(-50%)', background: 'radial-gradient(circle,rgba(124,58,237,.08) 0%,transparent 70%)' }} />
        <div className="container-md s-hide" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <p style={{ color: 'rgba(255,255,255,.6)', fontSize: '.97rem', maxWidth: 500, margin: '0 auto 36px' }}>
            Contact us for thread size, panel thickness compatibility, and bulk pricing.
          </p>
          <div className="d-flex gap-3 justify-content-center flex-wrap">
            ...
            <Link to="/self-clinching-fasteners" className="btn-outline-cyan" style={{ borderRadius: 60, padding: '14px 30px' }}>
              <i className="bi bi-arrow-left"></i> All Self-Clinching Fasteners
            </Link>
          </div>
        </div>
      </section>
