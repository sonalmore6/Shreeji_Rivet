...
function getAccent(product) {
  return product.id.startsWith('close') ? '#059669' : '#2563EB';
}
function getAccentLight(product) {
  return product.id.startsWith('close') ? 'rgba(5,150,105,.1)' : 'rgba(37,99,235,.1)';
}
function getAccentSoft(product) {
  return product.id.startsWith('close') ? '#6EE7B7' : '#93C5FD';
}

export default function RivetNutDetail() {
  const { id } = useParams();
  const product = rivetNutProducts.find((p) => p.id === id);
  ...
  if (!product) {
    return (
      <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 20, padding: 40 }}>
        ...
        <Link to="/rivet-nuts" className="btn-sm-blue">← Back to Rivet Nuts</Link>
      </div>
    );
  }

  const accent      = getAccent(product);
  const accentLight = getAccentLight(product);
  const accentSoft  = getAccentSoft(product);
  const isSealed    = product.id.startsWith('close');

  return (
    <div>
      <SEO
        title={`${product.name} — Rivet Nut`}
        description={product.desc}
        keywords={`${product.name}, Rivet Nut, Insert Nut, Nutsert, Shreeji Rivet India`}
        path={`/rivet-nuts/${product.id}`}
      ...
      <section style={{ background: DARK_BG, padding: '80px 0 70px', position: 'relative', overflow: 'hidden' }}>
        <div className="blob blob-abs" style={{ width: 500, height: 500, top: '-30%', right: '-8%', background: `radial-gradient(circle,${accentLight} 0%,transparent 70%)` }} />
        ...
        <div className="container-xl" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 28, fontSize: '.8rem', flexWrap: 'wrap' }}>
            ...
            <Link to="/rivet-nuts" style={{ color: '#38BDF8', textDecoration: 'none' }}>Rivet Nuts</Link>
            <span style={{ color: 'rgba(255,255,255,.3)' }}>/</span>
          ...
          <div className="row align-items-center g-5">
            ...
            <div className="col-12 col-md-5 s-hide-l">
              <div style={{
                ...
                boxShadow: `0 0 60px ${accentLight}`,
              }}>
                <img src={product.img} alt={product.name} style={{ maxHeight: 260, maxWidth: '100%', objectFit: 'contain', filter: `drop-shadow(0 8px 32px ${accentLight})` }} />
              </div>
            </div>
            ...
            <div className="col-12 col-md-7 s-hide-r">
              {/* Sealed badge */}
              {isSealed && (
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 14px', borderRadius: 50, background: 'rgba(5,150,105,.15)', border: '1px solid rgba(5,150,105,.3)', marginBottom: 14 }}>
                  <i className="bi bi-lock-fill" style={{ color: '#6EE7B7', fontSize: 11 }}></i>
                  <span style={{ color: '#6EE7B7', fontWeight: 700, fontSize: '.75rem', letterSpacing: '.06em' }}>SEALED / CLOSED END</span>
                </div>
              )}
              <div style={{ fontSize: '.78rem', color: accentSoft, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 14 }}>
                {product.badge}
              </div>
              ...
              <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
                ...
                <Link to="/rivet-nuts" style={{
                  ...
                  onMouseEnter={e => { e.currentTarget.style.borderColor = `${accent}80`; e.currentTarget.style.color = accentSoft; }}
            ...
            {product.specs.map((s, i) => (
              <div key={s.title} className={`col-12 col-md-4 s-hide d${i + 1}`}>
                <div style={{
                  ...
                  boxShadow: '0 4px 20px rgba(15,23,42,.06)', border: `1px solid ${accentLight}`,
                  transition: 'all .3s',
                }}
                  onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-6px)'; e.currentTarget.style.boxShadow = `0 20px 50px ${accentLight}`; e.currentTarget.style.borderColor = `${accent}40`; }}
                  onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 20px rgba(15,23,42,.06)'; e.currentTarget.style.borderColor = accentLight; }}
                >
                  <div style={{ width: 60, height: 60, borderRadius: 16, background: accentLight, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
            ...
            {product.features.map((f, i) => (
              <div key={f.title} className={`col-12 col-md-4 s-hide d${i + 1}`}>
                <div style={{
                  ...
                  onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-5px)'; e.currentTarget.style.borderColor = `${accent}40`; }}
                  onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = `${accent}18`; }}
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 14 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: accentLight, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2 }}>
                  ...
                  <div style={{ width: 36, height: 3, background: `linear-gradient(90deg,${accent},${accentSoft})`, borderRadius: 20, marginBottom: 14, marginLeft: 50 }} />
      ...
      <section style={{ background: DARK_BG, padding: '70px 0', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div className="blob2 blob-abs" style={{ width: 400, height: 400, top: '-40%', left: '50%', transform: 'translateX(-50%)', background: `radial-gradient(circle,${accentLight} 0%,transparent 70%)` }} />
        <div className="container-md s-hide" style={{ position: 'relative', zIndex: 1 }}>
          <h2 style={{ fontWeight: 900, color: '#fff', fontSize: 'clamp(1.6rem,3.5vw,2.4rem)', marginBottom: 12 }}>
            Interested in <span style={{ color: accentSoft }}>{product.name}?</span>
          </h2>
          <p style={{ color: 'rgba(255,255,255,.6)', fontSize: '.97rem', maxWidth: 500, margin: '0 auto 36px' }}>
            Contact us for thread size, material grade, and bulk pricing details.
          </p>
          <div className="d-flex gap-3 justify-content-center flex-wrap">
            ...
            <Link to="/rivet-nuts" className="btn-outline-cyan" style={{ borderRadius: 60, padding: '14px 30px' }}>
              <i className="bi bi-arrow-left"></i> All Rivet Nuts
            </Link>
          </div>
        </div>
      </section>
