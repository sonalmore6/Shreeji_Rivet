...
export default function BlindRivetDetail() {
  const { id } = useParams();
  const product = blindRivetProducts.find((p) => p.id === id);
  ...
  if (!product) {
    return (
      <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 20, padding: 40 }}>
        ...
        <Link to="/blind-rivets" className="btn-sm-blue">← Back to Blind Rivets</Link>
      </div>
    );
  }
  ...
  return (
    <div>
      <SEO
        title={`${product.name} — Blind Rivet`}
        description={product.desc}
        keywords={`${product.name}, Blind Rivet, POP Rivet, Aluminium Rivet, Shreeji Rivet India`}
        path={`/blind-rivets/${product.id}`}
        ...
        schema={{
          ...
          category: 'Blind Rivet',
        }}
      />

      {/* ══ HERO ══ */}
      <section style={{ background: DARK_BG, padding: '80px 0 70px', position: 'relative', overflow: 'hidden' }}>
        <div className="blob blob-abs" style={{ width: 500, height: 500, top: '-30%', right: '-8%', background: 'radial-gradient(circle,rgba(14,165,233,.15) 0%,transparent 70%)' }} />
        <div className="blob2 blob-abs" style={{ width: 350, height: 350, bottom: '-20%', left: '-5%', background: 'radial-gradient(circle,rgba(6,182,212,.1) 0%,transparent 70%)' }} />
        ...
        <div className="container-xl" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 28, fontSize: '.8rem', flexWrap: 'wrap' }}>
            ...
            <Link to="/blind-rivets" style={{ color: '#38BDF8', textDecoration: 'none' }}>Blind Rivets</Link>
            <span style={{ color: 'rgba(255,255,255,.3)' }}>/</span>
          ...
          <div className="row align-items-center g-5">
            ...
            <div className="col-12 col-md-5 s-hide-l">
              <div style={{
                ...
                boxShadow: '0 0 60px rgba(14,165,233,.08)',
              }}>
                <img src={product.img} alt={product.name} style={{ maxHeight: 260, maxWidth: '100%', objectFit: 'contain', filter: 'drop-shadow(0 8px 32px rgba(14,165,233,.2))' }} />
              </div>
            </div>

            {/* Product info */}
            <div className="col-12 col-md-7 s-hide-r">
              <div style={{ fontSize: '.78rem', color: '#38BDF8', fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 14 }}>
                {product.badge}
              </div>
              ...
              <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
                ...
                <Link to="/blind-rivets" style={{
                  ...
                  onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(56,189,248,.5)'; e.currentTarget.style.color = '#38BDF8'; }}
      ...
      <section style={{ padding: '80px 0', background: '#fff' }}>
        <div className="container-lg">
          <div className="text-center s-hide mb-5">
            ...
            <h2 className="section-title">Product <span style={{ color: '#0EA5E9' }}>Overview</span></h2>
          </div>
      ...
      <section style={{ padding: '80px 0', background: '#F0F7FF' }}>
        <div className="container-lg">
          <div className="text-center s-hide mb-5">
            ...
            <h2 className="section-title">Product <span style={{ color: '#0EA5E9' }}>Specifications</span></h2>
          </div>
            ...
            {product.specs.map((s, i) => (
              <div key={s.title} className={`col-12 col-md-4 s-hide d${i + 1}`}>
                <div style={{
                  ...
                  boxShadow: '0 4px 20px rgba(15,23,42,.06)', border: '1px solid rgba(14,165,233,.1)',
                  transition: 'all .3s',
                }}
                  onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-6px)'; e.currentTarget.style.boxShadow = '0 20px 50px rgba(14,165,233,.14)'; e.currentTarget.style.borderColor = 'rgba(14,165,233,.3)'; }}
                  onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 20px rgba(15,23,42,.06)'; e.currentTarget.style.borderColor = 'rgba(14,165,233,.1)'; }}
                >
                  <div style={{ width: 60, height: 60, borderRadius: 16, background: 'rgba(14,165,233,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
                    <i className={`bi ${s.icon}`} style={{ color: '#0EA5E9', fontSize: 26 }}></i>
                  </div>
                  <div style={{ fontWeight: 800, color: '#0F172A', fontSize: '.95rem', marginBottom: 10 }}>{s.title}</div>
                  <div style={{ fontWeight: 700, color: '#0EA5E9', fontSize: '1.15rem', marginBottom: 8 }}>{s.value}</div>
      ...
      <section style={{ padding: '80px 0', background: '#fff' }}>
        <div className="container-lg">
          <div className="text-center s-hide mb-5">
            ...
            <h2 className="section-title">Key <span style={{ color: '#0EA5E9' }}>Features</span></h2>
          </div>
          <div className="row g-4 justify-content-center">
            {product.features.map((f, i) => (
              <div key={f.title} className={`col-12 col-md-4 s-hide d${i + 1}`}>
                <div style={{
                  ...
                  onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-5px)'; e.currentTarget.style.borderColor = 'rgba(14,165,233,.35)'; }}
                  onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'rgba(14,165,233,.12)'; }}
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 14 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(16,185,129,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2 }}>
                      <i className="bi bi-check-circle-fill" style={{ color: '#10B981', fontSize: 18 }}></i>
                    </div>
                    <div style={{ fontWeight: 800, color: '#0F172A', fontSize: '.97rem', lineHeight: 1.3 }}>{f.title}</div>
                  </div>
                  <div style={{ width: 36, height: 3, background: 'linear-gradient(90deg,#0EA5E9,#38BDF8)', borderRadius: 20, marginBottom: 14, marginLeft: 50 }} />
      ...
      <section style={{ background: DARK_BG, padding: '70px 0', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container-md s-hide" style={{ position: 'relative', zIndex: 1 }}>
          <h2 style={{ fontWeight: 900, color: '#fff', fontSize: 'clamp(1.6rem,3.5vw,2.4rem)', marginBottom: 12 }}>
            Interested in <span style={{ color: '#38BDF8' }}>{product.name}?</span>
          </h2>
          ...
          <div className="d-flex gap-3 justify-content-center flex-wrap">
            ...
            <Link to="/blind-rivets" className="btn-outline-cyan" style={{ borderRadius: 60, padding: '14px 30px' }}>
              <i className="bi bi-arrow-left"></i> All Blind Rivets
            </Link>
          </div>
        </div>
      </section>
