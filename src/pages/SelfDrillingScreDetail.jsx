...
const ACCENT  = '#0EA5E9';
const SOFT    = '#0EA5E9';
const LIGHT   = 'rgba(5,150,105,.1)';

export default function SelfDrillingScreDetail() {
  const { id } = useParams();
  const product = selfDrillingScrewtProducts.find((p) => p.id === id);
  ...
  if (!product) {
    return (
      <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 20, padding: 40 }}>
        ...
        <Link to="/self-drilling-screws" className="btn-sm-blue">← Back to Self-Drilling Screws</Link>
      </div>
    );
  }
  ...
  return (
    <div>
      <SEO
        title={`${product.name} — Self Drilling Screw`}
        description={product.desc}
        keywords={`${product.name}, SDS, Self Drilling Screw, Self Tap Screw, Shreeji Rivet India`}
        path={`/self-drilling-screws/${product.id}`}
      ...
      <section style={{ background: DARK_BG, padding: '80px 0 70px', position: 'relative', overflow: 'hidden' }}>
        <div className="blob blob-abs" style={{ width: 500, height: 500, top: '-30%', right: '-8%', background: 'radial-gradient(circle,rgba(5,150,105,.15) 0%,transparent 70%)' }} />
        ...
        <div className="container-xl" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 28, fontSize: '.8rem', flexWrap: 'wrap' }}>
            ...
            <Link to="/self-drilling-screws" style={{ color: '#38BDF8', textDecoration: 'none' }}>Self-Drilling Screws</Link>
            <span style={{ color: 'rgba(255,255,255,.3)' }}>/</span>
          ...
          <div className="row align-items-center g-5">
            ...
            <div className="col-12 col-md-5 s-hide-l">
              <div style={{
                ...
                boxShadow: '0 0 60px rgba(5,150,105,.08)',
              }}>
                <img src={product.img} alt={product.name} style={{ maxHeight: 260, maxWidth: '100%', objectFit: 'contain', filter: 'drop-shadow(0 8px 32px rgba(5,150,105,.2))' }} />
              </div>
            </div>
            ...
            <div className="col-12 col-md-7 s-hide-r">
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 36 }}>
                {product.chips.map((chip) => (
                  <span key={chip} style={{
                    ...
                    background: LIGHT, border: `1px solid rgba(5,150,105,.3)`,
                    color: SOFT,
                  }}>{chip}</span>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
                ...
                <Link to="/self-drilling-screws" style={{
                  ...
                  onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(110,231,183,.5)'; e.currentTarget.style.color = SOFT; }}
            ...
            {product.specs.map((s, i) => (
              <div key={s.title} className={`col-12 col-md-4 s-hide d${i + 1}`}>
                <div style={{
                  ...
                  onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-6px)'; e.currentTarget.style.boxShadow = '0 20px 50px rgba(5,150,105,.12)'; e.currentTarget.style.borderColor = 'rgba(5,150,105,.35)'; }}
            ...
            {product.features.map((f, i) => (
              <div key={f.title} className={`col-12 col-md-4 s-hide d${i + 1}`}>
                <div style={{
                  ...
                  onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-5px)'; e.currentTarget.style.borderColor = 'rgba(5,150,105,.35)'; }}
                  onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'rgba(5,150,105,.12)'; }}
                >
      ...
      <section style={{ background: DARK_BG, padding: '70px 0', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div className="blob2 blob-abs" style={{ width: 400, height: 400, top: '-40%', left: '50%', transform: 'translateX(-50%)', background: 'radial-gradient(circle,rgba(5,150,105,.08) 0%,transparent 70%)' }} />
        <div className="container-md s-hide" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <p style={{ color: 'rgba(255,255,255,.6)', fontSize: '.97rem', maxWidth: 500, margin: '0 auto 36px' }}>
            Contact us for drill point selection, size range, coating options, and bulk pricing.
          </p>
          <div className="d-flex gap-3 justify-content-center flex-wrap">
            ...
            <Link to="/self-drilling-screws" className="btn-outline-cyan" style={{ borderRadius: 60, padding: '14px 30px' }}>
              <i className="bi bi-arrow-left"></i> All Self-Drilling Screws
            </Link>
          </div>
        </div>
      </section>
